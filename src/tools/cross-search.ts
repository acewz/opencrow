...
export function createCrossSourceSearchTool(
  ...
  return {
    ...
    description:
      "Search across ALL indexed sources in one call: news, Hacker News, Reddit, X/Twitter, Product Hunt, GitHub, arXiv, Semantic Scholar, HuggingFace. Returns results ranked by relevance with source labels. Use this instead of calling 5+ individual search tools when you need a broad sweep across sources.",
    inputSchema: {
      type: "object",
      properties: {
        query: {
        ...
        sources: {
          ...
          description:
            "Optional: limit to specific sources. Omit to search all. Values: article, story, reddit_post, tweet, product, github_repo, arxiv_paper, scholar_paper, hf_model.",
        },
        limit: {
          type: "number",
